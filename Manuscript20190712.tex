...
\title{An exact Riemann solver for one-dimensional multi-material elastic-plastic flows with Mie-Gr\"uneisen equation of state without vaccum}
...
\begin{abstract}
  ...
  In this paper, we present exact Riemann solvers for the Riemann problem and the half Riemann problem, respectively,  for one-dimensional multi-material elastic-plastic flows with the Mie-Gr\"uneisen equation of state(EOS), hypo-elastic constitutive model and  the von Mises' yielding condition. We first  analyze the Jacobian matrixes in  both elastic and plastic states, and then build relations between different variables across different type of waves. Based on these formulations, an exact Riemann solver is constructed with totally thirty-six possible cases of wave structures. {\color{red} In our work,  a four-equation Eulerian hyperbolic governing system is used and the elastic and plastic states are controlled by uniform equations and same EOS. The transformations of elastic and plastic states are  determined by the deviatoric stress spontaneously}. A large number of tests prove the rightness of our exact Riemann solver. Moreover, an exact Riemann solver is also deduced for the half Riemann problem and  its validity is tested  by two examples. 
\end{abstract}
...
In the development history of hydrodynamic numerical methods, the exact Riemann solver has played a very important role as it not only can give a guide and reference to the construction of approximate Riemann solvers, but also can be used to determine the convergence and stability of numerical schemes. However, building the exact Riemann solver for 1D elastic-plastic flows is not that easy. Comparing with the governing equations system of 1D pure fluids, for 1D elastic-plastic flows, there are two more equations: a non-conservative constitutive equation and the von Mises yielding condition.  The non-conservative character of the constitutive equation increases the difficulty in constructing  Riemann solvers, while the von Mises yielding condition may lead to more non-linear waves in the wave structures of Riemann solvers. Moreover, in a general way, the equation of state(EOS) for solid materials is more complex than that for pure fluids, which directly increases the difficulty in solving the Riemann problem.
...
{\color{red}
 Enlighen by but different from the works of Gao and Liu \cite{gao20171d,gao2018complete},
 in this paper, we want to construct an exact Riemann solver for the system of 1D elastic-plastic flows with the Mie-Gr\"uneisen EOS, the hypo-elastic constitutive model and the von Mises' yielding condition. Firstly, the elastic and plastic states are controlled by same equations but only different in the deviatoric stress, so there are  four equations in the governing system. Secondly, the same Mie-Gr\"uneisen EOS is used in both the elastic and plastic states.  As every knows, the Mie-Gr\"uneisen EOS is complex and is used widely in many engineering problems, which makes the energy equation must be included in the system of elastic-plastic flows and so we have to use some special methods to resolve the rarefaction wave.  Otherwise, we analyse the Jacobian matrix and find, in the wave structures of Riemann solvers, there may be three to five waves, including one contact wave and other two to four non-linear waves. These nonlinear waves may be elastic shock waves, elastic rarefaction waves,  plastic shock waves or plastic rarefaction waves. Moreover, elastic waves always runs faster than the followed plastic wave. So there are thirty-six possible cases of the wave structures in the Riemann solver.  Besides, both the changing from elastic to plastic and  the converse changing from plastic to elastic  are considered in the Riemann problem.}
...
If the material is in the plastic state, the above equations can be simplified
...
\begin{enumerate}[Step 1]
  ...
  After determining the structures case, we need  to solve  Cauchy stresses and velocities in regions $\mathbf{Q}^*_L$ and $\mathbf{Q}^*_R$, those are given in Section \ref{sec:functions}.
...
\subsection{Test 1}
In this case, the material is yielding at both sides, so the solution structures has three waves with two plastic shock waves and one contact.
...
\subsection{Test 3}
In this example, we test the case with two elastic rarefaction waves. In the wave structures there is one elastic rarefaction wave on each side of the contace wave. The initial condition is given as
\begin{equation}
 \left\{ \begin{aligned}
...
\begin{equation}
 ...
 \right.
\end{equation}
Different from Test 5, in  the initial condition, at the beginning, the material is in the positive  plastic state (i.e. $s_{xxL(R)}=\frac{2}{3}{Y}^{0}$). According to the analysis introduced in Section \ref{sec:relation}, after an compressing  process the material will turn back into an elastic state and will yield again into a negtive plastic state, we can get the exact  solutions match the numerical solutions very well by the new Riemann solver in Fig.\ref{fig:case21}.
...
\begin{figure}[ht]
 ...
 \includegraphics[width= 7cm] {case21rho.pdf}
  ...
  \includegraphics[width= 7cm] {case21sxx.pdf}
    \caption{Comparison results for Test 6 with the structures of $S^ES^P|S^PS^E$.  }
  \label{fig:case21}
\end{figure}
...
\begin{figure}[ht]
  ...
  \includegraphics[width= 7cm] {case5sxx.pdf}
    ...
    \caption{Comparison resutls for Test 7 with the structures of $R^P|R^PR^E$.  }
  \label{fig:case5}
\end{figure}
\subsection{Test 8}
...
\begin{figure}[ht]
  ...
  \includegraphics[width= 7cm] {case9sxx.pdf}
    ...
    \caption{Comparison resutls for Test 8 with the structures of $R^ER^P|S^PS^E$.  }
  \label{fig:case9}
\end{figure}
\subsection{Test 9}
...
\begin{figure}[ht]
  ...
  \includegraphics[width= 7cm] {case6sigma.pdf}
    ...
    \caption{Comparison results for Test 9 with the structures of $R^ER^P|R^PR^E$.  }
  \label{fig:case7}
\end{figure}
...
\begin{figure}[ht]
  ...
  \includegraphics[width= 7cm] {case8sigma.pdf}
    ...
    \caption{Comparison results for Test 10 with the structures of $R^ER^P|S^ES^P$.  }
  \label{fig:case8}
\end{figure}
...
\begin{figure}[ht]
  ...
  \includegraphics[width= 7cm] {case10sxx.pdf}
    ...
    \caption{Comparison results for Test 11 with the structures of $S^ES^P$.  }
  \label{fig:case9}
\end{figure}
